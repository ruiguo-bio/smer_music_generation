...
def occupation_polyphony_rate(pm,track_length):
    occupation_rate = []
    polyphony_rate = []
    beats = pm.get_beats()
    fs = 4 / (beats[1] - beats[0])
    pm.get_beats()
    total_roll = pm.get_piano_roll(fs=fs)
...
def remove_continue_add_control_event(file_events,header_events,key,tensiles,diameters,add_control=False,rest_multi=True,remove_continue=False):
    ...
    if add_control:
        ...
        bar_length = int(str(header_events[0])[0])

        if bar_length != 6:
            track_length = bar_length * 4
            total_track_length = track_length * len(bar_pos)

        else:
            track_length = bar_length / 2 * 4
            total_track_length = track_length * len(bar_pos)
        ...
        occupation_rate, polyphony_rate = occupation_polyphony_rate(pm,track_length)
        ...
        if tensiles is not None:
            ...
            tension_positions = np.where(np.array(new_file_events) == 'track_0')[0]
...
#
#
def cal_separate_file(files,i,augment=False,add_control=False,rest_multi=True):
    ...
    for pos in range(len(bar_beginning_pos) - 1):
        ...
        if add_control:
            ...
            if pos == len(bar_beginning_pos) - 2:
                ...
                return_events = remove_continue_add_control_event(file_events[bar_beginning_pos[pos]:], header_events, key,
                                                                  tensiles[tension_pos:],diameters[tension_pos:], add_control=add_control,rest_multi=rest_multi,remove_continue=is_continue)

            else:
                return_events = remove_continue_add_control_event(file_events[bar_beginning_pos[pos]:bar_beginning_pos[pos + 2]],
                                                   header_events, key,tensiles[tension_pos:tension_pos+16],diameters[tension_pos:tension_pos+16], add_control=add_control,rest_multi=rest_multi,remove_continue=is_continue)
        else:
        ...
        if return_events is not None:
            ...
            logger.info(f'skip file {i} bar pos {pos}')
...
def gen_batches(files,max_token_length=2200,augment=False,add_control=False,rest_multi=True):
    ...
    # return_events = Parallel(n_jobs=8)(delayed(cal_separate_file)(files,i,augment=augment,add_control=add_control,rest_multi=rest_multi) for i in range(0,len(files)))

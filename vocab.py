...
class WordVocab(object):
    def __init__(self, mode,control_list):
        ...
        if mode == 0:
            ...
            duration_tokens = duration_only_token + rest + sep + ['continue']
        ...
        if mode == 0:
            self.rest_indices = [self._char2idx[name] for name in rest]
            self.sep_indices = [self._char2idx[name] for name in sep]
        ...
        for index in self.rest_indices:
            self.token_class_ranges[index] = 'rest'
            if 'rest' in self.name_to_tokens:
                self.name_to_tokens['rest'].append(self._idx2char[index])
            else:
                self.name_to_tokens['rest'] = [self._idx2char[index]]

        for index in self.sep_indices:
            self.token_class_ranges[index] = 'sep'
            if 'sep' in self.name_to_tokens:
                self.name_to_tokens['sep'].append(self._idx2char[index])
            else:
                self.name_to_tokens['sep'] = [self._idx2char[index]]
